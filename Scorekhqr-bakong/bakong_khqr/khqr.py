...
class KHQR:
    ...
    def check_payment(
        ...
        # If responseCode == 0, transaction is successfully PAID
        if response.get("responseCode") == 0:
            return "PAID"
        
        # Check if rate limit exceeded or token issue
        error_code = response.get("errorCode")
        response_msg = response.get("responseMessage", "")
        if error_code == 17 or "limit" in response_msg.lower() or "exceeded" in response_msg.lower():
            raise ValueError(f"Bakong Rate Limit: {response_msg}")
        if "token" in response_msg.lower() or "unauthorized" in response_msg.lower():
            raise ValueError(f"Bakong Token Error: {response_msg}")
