...
# Add to interface
if 'Task<string> CompletePreAuthAsync' not in text:
    old_interface = 'Task<string> GetPaymentLinkDetailsAsync(string linkId);'
    new_interface = 'Task<string> GetPaymentLinkDetailsAsync(string linkId);\n        Task<string> CompletePreAuthAsync(string tranId, decimal completeAmount);'
    text = text.replace(old_interface, new_interface)

new_method = """
        public async Task<string> CompletePreAuthAsync(string tranId, decimal completeAmount)
        {
...
if 'public async Task<string> CompletePreAuthAsync' not in text:
    text = text.rstrip()
...
new_ctrl = """
    ...
    [AllowAnonymous]
    public async Task<IActionResult> CompletePreAuth([FromBody] dynamic payload)
    {
...
if 'CompletePreAuth(' not in ctrl_text:
