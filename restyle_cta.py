...
pattern_cta = re.compile(r'\{\/\* Desktop Top Up CTA \*\/\}.*?<\/button>\s*\}\)\s*<\/div>\s*<\/div>\s*<\/div>', re.DOTALL)

new_cta = """{/* Desktop Top Up CTA */}
              <Link
                to="/topup"
...
text, count = pattern_cta.subn(new_cta, text)
print(f"Replaced {count} instances of CTA block.")
