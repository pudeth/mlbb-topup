...
pattern_right = re.compile(r'\{\/\* Desktop Top Up CTA \*\/\}.*?<\/div>\s*<\/div>\s*<\/header>', re.DOTALL)

new_right = """{/* Desktop Top Up CTA */}
              <Link
                to="/topup"
                className="hidden lg:flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 text-xs font-black tracking-wide rounded-full shadow-[0_0_15px_rgba(251,191,36,0.3)] transition-all duration-300 hover:scale-105"
              >
...
text, count = pattern_right.subn(new_right, text)
print(f"Replaced {count} instances of right nav.")
